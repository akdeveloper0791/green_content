...
from .models import Player as groupPlayer
@api_view(["POST"])
def assignPlayersGCGroup(request):
    if(request.method == 'POST'):
        isWeb = False;
        accessToken = request.POST.get('accessToken');
        if(accessToken == 'web'):
        ...
        result = groupPlayer.assignNew(
            accessToken,request.POST.get('g_id'),request.POST.get('players'),isWeb);
...
@api_view(["POST"])
def removePlayersGCGroup(request):
    if(request.method == 'POST'):
        isWeb = False;
        accessToken = request.POST.get('accessToken');
        if(accessToken == 'web'):
